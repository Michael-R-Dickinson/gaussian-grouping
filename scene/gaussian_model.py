...
class GaussianModel:
    ...
    def load_ply_foreign(self, path, spatial_lr_scale):
        """Warm-start from a plain 3DGS point cloud (no obj_dc_* fields).

        Identity-encoding features are randomly initialized since the source
        scene was reconstructed without Gaussian Grouping's object head.
        """
        self.spatial_lr_scale = spatial_lr_scale
        plydata = PlyData.read(path)
        ...
        # Foreign ply has no identity-encoding channels; random-init like create_from_pcd does.
        objects_dc = RGB2SH(torch.rand((xyz.shape[0], self.num_objects), device="cuda")).cpu().numpy()[:, :, None]
